 ...
 * A phone is held closer but carries a fraction of the fill rate, and this
 * scene costs three passes over the geometry — occlusion and two outlines —
 * before it reaches the screen. 1.5 renders 44% fewer pixels than 2 for a
 * softening that is hard to see at arm's length and easy to feel in the hand.
 */
export function pixelRatioCap() {
  const device = globalThis.devicePixelRatio ?? 1;
  return Math.min(device, isPhone() ? 1.5 : 2);
}
